...
def normalize_pb_name(pb_name):
    """ Some pb_type names generated by the tool
...
def normalize_cell_name(cell_name):
    """ Some cell names might be created from generate statement.
        These cells correspond to the same pb_type, therefore, even though
        their names are different, they should be noramlized to reference
        the same exact pb type.

        This function takes the cell_name as input and removes the
        index assigned by the generate statement.

        output_dff[0] -> output_dff
        output_dff[1] -> output_dff
    """

    # Apply first normalization step
...
def mark_all_paths(interconnects):
    """
    This function marks all the paths that require a pack pattern.

    The current pb_type level connections between IO of the children cells
    are identified and all the required pack patterns added to the
    input interconnects dictionary.
    """

    def find_path(paths, driver_cell):
        """
        This function finds the path starting with the input driver cell
        among all the paths passed as argument
        """
        for path in paths:
            cell, _ = path[0]
            if cell == driver_cell:
    ...
    def all_paths_found(paths, driver_cells):
        """
        This function checks whether there are any more paths to be
        discovered.

        If the last element of a path of all the currently discovered
        paths is not a driver cell of any other path, True is returned.
        """
        for path in paths:
    ...
    # Initial discovery of potential paths
    ...
    # Discover new paths iteratively
    ...
    # For each path, generate a pack pattern
    marked_cells = dict()
    for path in paths:
        cells, pins = zip(*path)
    ...
    # Replace all the pack patterns with the newly disovered if the
    # pack pattern is required for a given sink connection
    for (drv_cell, drv_pin), sink_list in interconnects.items():
...
def make_container_pb(
    ...
    for child_prefix, (child_type, children_names) in children.items():
        ...
        with open(pb_type_path, 'r') as inc_xml:
            ...
            # Create objects to inspect the children pb_types
            pb_node = PbTypeNode(
                normalized_name,
    ...
    for (driver_cell, driver_pin), sinks in interconn.items():
        ...
        for (sink_cell, sink_pin), path_attr in non_routing_sinks:
            ...
            # When a pack attribute is found, we need to add, if necessary
            # all the pack patterns to the children pb_types as well
